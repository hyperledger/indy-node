...
def test_plugin_or_rule_one_amount_same_role_trustee_no_endorser(write_auth_req_validator, write_request_validation,
                                                                 signatures, is_owner, amount):
    validate(
        auth_constraint=AuthConstraintOr(auth_constraints=[
        ...
        valid_actions=[Action(author=TRUSTEE, endorser=None, sigs={TRUSTEE: s1, ENDORSER: s2},
                              is_owner=owner, amount=2, extra_sigs=True)
                       for s1 in range(1, 4)
                       for s2 in range(1, 4)
                       for owner in [True, False]] +
...
def test_plugin_or_rule_one_amount_diff_roles_trustee_no_endorser(write_auth_req_validator, write_request_validation,
                                                                  signatures, is_owner, amount):
    validate(
        auth_constraint=AuthConstraintOr(auth_constraints=[
            AuthConstraint(role=ENDORSER, sig_count=1, need_to_be_owner=False),
        ...
        valid_actions=[Action(author=TRUSTEE, endorser=None, sigs={TRUSTEE: s1, ENDORSER: s2},
                       ...
                       for s2 in range(1, 4)
                       for owner in [True, False]] +
                      [Action(author=TRUSTEE, endorser=None, sigs={TRUSTEE: s1, IDENTITY_OWNER: s2},
