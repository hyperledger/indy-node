...
class GetNymHandler(ReadRequestHandler):
    ...
    def get_result(self, request: Request):
        ...
        if timestamp and read_seq_no:
            ...
            # if txn and "result" in txn:
                # timestamp = get_txn_time(txn.result)  # this is the problem
                # nym_data, proof = self._get_value_by_timestamp_from_state(path, timestamp=timestamp, with_proof=False)
            nym_data, proof = self._get_value_by_seq_no_from_state(path, read_seq_no, request, with_proof=False)
    ...
    def _get_value_by_seq_no_from_state(
        ...
        return self._get_value_by_timestamp_from_state(path, timestamp=timestamp, with_proof=False)
