...
# Can't find how to replace abbreviate_verkey need to findd out if the call still exist within new packages
from indy.did import abbreviate_verkey
...
def add_new_nym(looper, sdk_pool_handle, creators_wallet, dest, verkey=None):
    ...
    nym_request = looper.loop.run_until_complete(ledger.build_nym_request(submitter_did, dest, verkey, None, None))
...
def nym_full_vk(looper, tconf, nodeSet, sdk_pool_handle, sdk_wallet_trustee):
    ...
    (new_did, new_verkey) = looper.loop.run_until_complete(
        create_and_store_did(wh, json.dumps({'seed': randomString(32)})))
...
def nym_get(looper, sdk_pool_handle, sdk_wallet_sign, did):
    ...
    get_nym_req = looper.loop.run_until_complete(ledger.build_get_nym_request(sign_did, did))
