...
from indy_common.constants import RS_CONTENT, JSON_LD_CONTEXT
...
@pytest.fixture()
def context_handler():
    return JsonLdContextHandler(None, None)


def test_static_validation_context_fail_bad_uri(context_handler, context_req):
    context_req.operation[RS_CONTENT] = json.dumps({JSON_LD_CONTEXT: "2http:/..@#$"})
...
def test_static_validation_fail_context_not_uri_or_array_or_object(context_handler, context_req):
    context_req.operation[RS_CONTENT] = json.dumps({JSON_LD_CONTEXT: 52})
    with pytest.raises(InvalidClientRequest) as e:
        context_handler.static_validation(context_req)
...
def test_static_validation_pass_context_value_is_dict(context_handler, context_req):
    ...
    context_req.operation[RS_CONTENT] = json.dumps({JSON_LD_CONTEXT: context})
    context_handler.static_validation(context_req)
