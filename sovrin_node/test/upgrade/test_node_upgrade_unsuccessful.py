...
def tdirWithPoolTxns(tdirWithPoolTxns, poolTxnNodeNames, tconf):
    ...
    populate_log_with_upgrade_events(tdirWithPoolTxns, poolTxnNodeNames, tconf, INVALID_VERSION)
    return tdirWithPoolTxns


def test_node_handles_unsuccessful_upgrade(looper, nodeSet, nodeIds):
    check_node_set_acknowledges_upgrade(looper, nodeSet, nodeIds, [IN_PROGRESS, FAIL], INVALID_VERSION)
