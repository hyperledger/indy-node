...
def test_only_trustee_send_pool_config_writes_true_force_false(nodeSet, tdir, looper, trustee,
                                                               trusteeWallet, poolConfigWTFF):
...
def test_only_trustee_send_pool_config_writes_false_force_false(nodeSet, tdir, looper, trustee,
                                                                trusteeWallet, poolConfigWFFF):
    stClient, stWallet = getClientAddedWithRole(nodeSet, tdir, looper, trustee, trusteeWallet, 'tmpname', STEWARD)
    _, req = sendPoolConfig(stClient, stWallet, poolConfigWFFF)
    looper.run(eventually(checkRejects, stClient, req.reqId, 'cannot do'))


def test_only_trustee_send_pool_config_writes_true_force_true(nodeSet, tdir, looper, trustee,
                                                              trusteeWallet, poolConfigWTFT):
    stClient, stWallet = getClientAddedWithRole(nodeSet, tdir, looper, trustee, trusteeWallet, 'tmpname', STEWARD)
    _, req = sendPoolConfig(stClient, stWallet, poolConfigWTFT)
    looper.run(eventually(checkNacks, stClient, req.reqId, 'cannot do'))


def test_only_trustee_send_pool_config_writes_false_force_true(nodeSet, tdir, looper, trustee,
                                                               trusteeWallet, poolConfigWFFT):
    stClient, stWallet = getClientAddedWithRole(nodeSet, tdir, looper, trustee, trusteeWallet, 'tmpname', STEWARD)
